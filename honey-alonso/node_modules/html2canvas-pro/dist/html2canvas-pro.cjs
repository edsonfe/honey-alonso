/*!
* html2canvas-pro 2.4.4 <https://yorickshan.github.io/html2canvas-pro/>
...
const fontFamily = {
	...
	parse: (_context, tokens) => {
		...
		return results.map((result, index) => quoted[index] || result.indexOf(" ") !== -1 ? `'${result.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/\n/g, "\\a ").replace(/\r/g, "\\d ").replace(/\f/g, "\\c ")}'` : result);
	}
};
//#endregion
...
const renderFilterArgs = (values) => {
	...
	for (const v of values) {
		...
		if (v.type === 18) parts.push(`${v.name}(${renderFilterArgs(v.values)})`);
		else if (v.type === 4) parts.push(",");
		else if (v.type === 6) parts.push(v.value);
		else if (v.type === 15) parts.push(`${v.number}${v.unit}`);
...
const createsRealStackingContext = (node, container, root) => {
	return container.styles.isPositionedWithZIndex() || container.styles.opacity < 1 || Boolean(container.styles.filter) || container.styles.isTransformed() || isBodyElement(node) && root.styles.isTransparent();
};
...
var ElementPaint = class {
	...
	getEffects(target, surfaceRoot) {
		let inFlow = [2, 3].indexOf(this.container.styles.position) === -1;
		let parent = this === surfaceRoot ? null : this.parent;
		const sourceEffects = (paint) => paint.effects.filter((effect) => paint !== surfaceRoot || !isFilterEffect(effect) && !isOpacityEffect(effect));
...
//#endregion
//#region src/render/canvas/filter-surface.ts
var FilterSurfaceError = class extends Error {};
const releaseSurface = (canvas) => {
...
function filterOutset(filter) {
	return Math.ceil(3 * (filter.blur + (filter.shadow?.blur ?? 0)) + Math.max(Math.abs(filter.shadow?.x ?? 0), Math.abs(filter.shadow?.y ?? 0)));
}
...
const decodeSurface = (image, signal) => new Promise((resolve, reject) => {
	const finish = (error) => {
		clearTimeout(timer);
	...
	const timer = setTimeout(() => finish(new FilterSurfaceError("Filter surface image decode timed out")), 1e4);
	signal?.addEventListener("abort", abort, { once: true });
	if (signal?.aborted) abort();
	else image.decode().then(() => finish(), () => finish(new FilterSurfaceError("Filter surface image decode failed")));
});
const canvasFilterSupport = /* @__PURE__ */ new WeakMap();
function supportsNativeFilters(owner = document) {
...
function tryNativeFilterSurface(source, { blur, shadow }, opacity, scale, signal) {
	let filtered;
	let output;
	let complete = false;
	try {
		...
		if (shadow) functions.push(`drop-shadow(${shadow.x * scale}px ${shadow.y * scale}px ${shadow.blur * scale}px ${shadow.color})`);
...
async function renderFilterSurface(source, filter, opacity, scale, signal) {
	throwIfAborted(signal);
...
async function renderSvgFilterSurface(source, { blur, shadow }, opacity, scale, signal) {
	throwIfAborted(signal);
	if (!blur && !shadow) {
	...
	try {
		...
		throw error instanceof FilterSurfaceError ? error : new FilterSurfaceError("Filter surface image cannot be drawn");
	} finally {
		image.removeAttribute("src");
	}
}
function reserveSurface(budget, width, height) {
	const pixels = width * height * 4;
	if (!Number.isFinite(pixels) || width <= 0 || height <= 0 || width > 8192 || height > 8192 || budget.pixels + pixels > 16777216) return 0;
	budget.pixels += pixels;
	return pixels;
}
const expand = (b, margin) => b.add(-margin, -margin, margin * 2, margin * 2);
const union = (a, b) => new Bounds(Math.min(a.left, b.left), Math.min(a.top, b.top), Math.max(a.left + a.width, b.left + b.width) - Math.min(a.left, b.left), Math.max(a.top + a.height, b.top + b.height) - Math.min(a.top, b.top));
function surfaceBounds(container) {
	const styles = container.styles;
	let bounds = container.bounds;
	for (const shadow of styles.boxShadow) if (!shadow.inset) {
		const spread = Math.max(0, shadow.spread.number) + 3 * shadow.blur.number;
		bounds = union(bounds, expand(container.bounds.add(shadow.offsetX.number, shadow.offsetY.number, 0, 0), spread));
	}
	for (const text of container.textNodes) for (const part of text.textBounds) {
		const ink = expand(part.bounds, 2 * getAbsoluteValue(styles.fontSize, container.bounds.height) + styles.webkitTextStrokeWidth);
		bounds = union(bounds, ink);
		for (const shadow of styles.textShadow) bounds = union(bounds, expand(ink.add(shadow.offsetX.number, shadow.offsetY.number, 0, 0), 3 * shadow.blur.number));
	}
	for (const child of container.elements) if (child.styles.isVisible()) bounds = union(bounds, surfaceBounds(child));
	const filter = parseSimpleFilter(styles.filter);
	return expand(bounds, filter ? filterOutset(filter) : 0);
}
function cropSurface(bounds, viewport, margin, scale) {
...
var CanvasRenderer = class CanvasRenderer {
	...
	async renderStack(stack) {
		...
		if (styles.isVisible()) {
			const filter = parseSimpleFilter(styles.filter);
			if (stack.element !== this.surfaceRoot && !this.legacySubtree && filter && (styles.filter || styles.opacity < 1) && this.canComposite(stack.element)) {
	...
	canComposite(paint) {
		const supported = (container) => {
			const styles = container.styles;
			return !styles.isTransformed() && styles.zoom === 1 && styles.mixBlendMode === MIX_BLEND_MODE$1.NORMAL && styles.clipPath.type === 0 && !contains(styles.display, 2048) && parseSimpleFilter(styles.filter) !== null;
		};
	...
	async renderCompositedStack(stack, filter) {
		...
		if (signal?.aborted) throw new DOMException("The operation was aborted.", "AbortError");
		const bounds = cropSurface(surfaceBounds(stack.element.container), new Bounds(this.options.x, this.options.y, this.options.width, this.options.height), filterOutset(filter), this.options.scale);
		if (!bounds.width || !bounds.height) return true;
		const reserved = reserveSurface(this.surfaceBudget, Math.ceil(bounds.width * this.options.scale), Math.ceil(bounds.height * this.options.scale));
		if (!reserved) {
		...
		try {
			...
			filtered = await renderFilterSurface(source.canvas, filter, stack.element.container.styles.opacity, options.scale, signal);
			if (signal?.aborted) throw new DOMException("The operation was aborted.", "AbortError");
			const own = stack.element.effects;
			const effects = stack.element.getEffects(4, this.surfaceRoot).filter((effect) => !own.includes(effect) || !isFilterEffect(effect) && !isOpacityEffect(effect) && !isClipEffect(effect));
	...
	*/
	...
	async renderNodeContent(paint) {
		this.effectsRenderer.applyEffects(paint.getEffects(4, this.surfaceRoot));
	...
	async renderSurfaceBoxShadow(paint, shadow) {
		...
		const bounds = cropSurface(box.add(-margin, -margin, 2 * margin, 2 * margin), new Bounds(this.options.x - shadow.offsetX.number, this.options.y - shadow.offsetY.number, this.options.width, this.options.height), margin, scale);
		...
		try {
			...
			createCanvasPath(ctx, transformPath(calculateBorderBoxPath(paint.curves), -spread, -spread, 2 * spread, 2 * spread));
	...
	async renderNodeBackgroundAndBorders(paint) {
		this.effectsRenderer.applyEffects(paint.getEffects(2, this.surfaceRoot));
