...
exports.releaseSurface = exports.FilterSurfaceError = void 0;
exports.parseSimpleFilter = parseSimpleFilter;
exports.filterOutset = filterOutset;
exports.supportsNativeFilters = supportsNativeFilters;
exports.renderFilterSurface = renderFilterSurface;
exports.renderSvgFilterSurface = renderSvgFilterSurface;
const effects_1 = require("../effects");
const abort_helper_1 = require("../../core/abort-helper");
// Only errors in the optional surface path are eligible for legacy fallback.
class FilterSurfaceError extends Error {
}
exports.FilterSurfaceError = FilterSurfaceError;
const releaseSurface = (canvas) => {
    canvas.width = canvas.height = 0;
};
exports.releaseSurface = releaseSurface;
...
function parseSimpleFilter(value) {
    if (!value || value === 'none')
        return { blur: 0 };
    const effect = new effects_1.FilterEffect(value);
    const shadows = value.match(/drop-shadow\(/g) || [];
    if (shadows.length > 1 || (shadows.length === 1 && !effect.shadow))
        return null;
    const blur = effect.safeFilterString.match(/^blur\((\d+(?:\.\d+)?)px\)$/);
    if (effect.safeFilterString && !blur)
        return null;
    if (blur && shadows.length && value.indexOf('blur(') > value.indexOf('drop-shadow('))
        return null;
    return {
...
function filterOutset(filter) {
    return Math.ceil(3 * (filter.blur + (filter.shadow?.blur ?? 0)) +
...
const decodeSurface = (image, signal) => new Promise((resolve, reject) => {
    ...
    if (signal?.aborted)
        ...
        image.decode().then(() => finish(), () => finish(new FilterSurfaceError('Filter surface image decode failed')));
});
...
const canvasFilterSupport = new WeakMap();
function supportsNativeFilters(owner = document) {
    ...
    try {
        ...
        (0, exports.releaseSurface)(source);
        (0, exports.releaseSurface)(target);
    }
...
function tryNativeFilterSurface(source, { blur, shadow }, opacity, scale, signal) {
    ...
    try {
        ...
        const functions = [];
        if (blur)
            functions.push(`blur(${blur * scale}px)`);
        if (shadow)
            functions.push(`drop-shadow(${shadow.x * scale}px ${shadow.y * scale}px ${shadow.blur * scale}px ${shadow.color})`);
        ...
        if (filtered)
            (0, exports.releaseSurface)(filtered);
        if (output && !complete)
            (0, exports.releaseSurface)(output);
    }
}
...
async function renderSvgFilterSurface(source, { blur, shadow }, opacity, scale, signal) {
    ...
    if (shadow) {
        ...
        filter.appendChild(element('feOffset', { dx: shadow.x * scale, dy: shadow.y * scale, result: 'offset-shadow' }));
    ...
    try {
        ...
        if (sourceUrl === 'data:,')
            throw new Error('Canvas size is unsupported');
    }
    catch {
